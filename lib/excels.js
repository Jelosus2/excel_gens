...
module.exports = {
    ...
    weaponMeta: (wl, ws) => {
        ...
        for (const wlObj of wl)
            json.levels.push({ Rarity: wlObj[RARITY], Level: wlObj[LEVEL], Field_XXX: wlObj[FIELD_XXX] });
        for (const wsObj of ws)
            json.breakLevels.push({ Rarity: wsObj[RARITY], BreakLevel: wsObj[BREAK_LEVEL], Field_YYY: wsObj[FIELD_YYY], Field_ZZZ: wsObj[FIELD_ZZZ] });
