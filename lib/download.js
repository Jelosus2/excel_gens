...
module.exports = {
  ...
  downloadSkillTree: async () => {
    ...
    Object.keys(diffs).forEach(async (key) => {
      const res = await fetch(`https://raw.githubusercontent.com/FortOfFans/HSRMaps/master/${key}/avatar.json`);
      ...
      Object.keys(data).forEach((characterId) => {
        data[characterId].tree?.forEach((branch) => {
          json[branch.id] = branch;
          json[branch.id].avatarName = data[characterId].name || "unkown";
        });
      });

      fs.writeFile(`./data/SkillTree/avatartree_${diffs[key]}.json`, JSON.stringify(json, null, 2), { flag: 'w' }, (err) => {
        if (err) console.error(err);
